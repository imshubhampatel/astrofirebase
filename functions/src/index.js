    ...
    .then((moneyRef) => {
      ...
      if (subtype == "meeting") {
        db.collection("meetings")
          .doc(subtypeId)
          .update({
            astrologerAmount: admin.firestore.FieldValue.increment(
              +astrologerAmount
            ),
          });
      }
...
app.post("/intiatetransaction_razorypay/", async (req, res) => {
  ...
  const instance = new Razorpay({
    key_id: "rzp_live_qQu2IyydGrZJfS",
    key_secret: "qBHeVXImlQZ8JbZ2FS8fAZDN",
  });
...
app.post("/razor_capture/:paymentId", (req, res) => {
  ...
  let config = {
    RAZOR_PAY_KEY_ID: "rzp_live_qQu2IyydGrZJfS",
    RAZOR_PAY_KEY_SECRET: "qBHeVXImlQZ8JbZ2FS8fAZDN",
  };
  console.log("called");
                          ...
                          .then(async (useRef) => {
                            ...
                            if (useData != null) {
                              if (useData.useCount < couponData.limit) {
                                if (
                                  useData.totalDiscount + discount >
        ...
        admin
          ...
          .then((userRef) => {
            ...
            fcm_notification(
              ...
              userData.tokens[userData.tokens.length - 1],
              "Meeting accepted",
              "Your request for meeting is accepted",
  ...
  .onUpdate(async (change, context) => {
    ...
    if (originalData.status != updatedData.status) {
      if (updatedData.status == "approved") {
        try {
          if (
            (updatedData.transactionId != null) &
        ...
        astrologerSnap.docs.map((doc) => {
          admin
            ...
            .then((astrologerRef) => {
              astrologerData = astrologerRef.data();
              if (
                astrologerData.pricingCategory == context.params.pricingCategory
              ) {
                _datarwt.push(
                  admin
                    .firestore()
