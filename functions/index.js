...
function addMoneyToWallet(user,amount,orderID) {
  admin.firestore().collection('app_details').doc("money").get().then((moneyRef) => {
    ...
    admin.firestore().collection("user").doc(user).update({"walletBalance": admin.firestore.FieldValue.increment(+userAmount)});
    admin.firestore().collection('user').doc(user).collection("wallet_transaction").add({"subtypeId":orderID,"amount":+userAmount,"type":"credit","subtype":"payment","date":admin.firestore.FieldValue.serverTimestamp()});
  });
}
