...
app.post('/updatetransaction_razorpay/', async (req, response) => {
    ...
    if (req.body &&
        ...
        req.body["ORDERID"] &&
        req.body["TXNAMOUNT"]) {
        const writeResult1 = await admin.firestore().collection('payments').doc(req.body['ORDERID']).set({ "user": req.body['USER'], "order": req.body['ORDERID'], status: "Initiated", "amount": req.body['TXNAMOUNT'] });
    }
    else {
    ...
    admin.firestore().collection('payments').doc(req.body['ORDERID']).get().then((paymentRef) => {
        ...
        if (req.body['STATUS'] == 'TXN_SUCCESS') {
            ...
            admin.firestore()
                .collection('payments')
                .doc(req.body['ORDERID'])
                .update({ "invoiceNo": +invoiceNo, "status": req.body['STATUS'], "txnId": req.body['ORDERID'] });
            admin.firestore().collection('app_details').doc("money").get().then((moneyRef) => {
                moneyData = moneyRef.data();
                let userAmount = amount * ((100) / (100 + moneyData.gst));
                let gst = amount - userAmount;
