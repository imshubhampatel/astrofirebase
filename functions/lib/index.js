...
function updateAstrologerBalance(astrologerId, totalAmount, subtypeId, subtype) {
    admin
        ...
        let status1 = db
            ...
            .doc(astrologerId)
            .update({
            walletBalance: admin.firestore.FieldValue.increment(+astrologerAmount),
        });
        let status = db
            .collection("astrologer")
            .doc(astrologerId)
        ...
        db.collection("app_details")
            .doc("money")
            .update({
            adminBalance: admin.firestore.FieldValue.increment(+adminAmount),
        });
        db.collection("app_details")
            .doc("money")
            .collection("admin_wallet_transaction")
            .add({
            subtypeId: subtypeId,
            amount: +adminAmount,
            type: "credit",
        ...
        if (subtype == "meeting") {
            db.collection("meetings")
                .doc(subtypeId)
                .update({
                astrologerAmount: admin.firestore.FieldValue.increment(+astrologerAmount),
            });
        }
    });
}
...
function updateMeetingMetrics(astrologerId, userId, totalDuration, type) {
    if (type == "chat") {
        admin
            ...
            .doc(astrologerId)
            .update({
            chatSeconds: admin.firestore.FieldValue.increment(totalDuration * 60),
        });
        admin
            .firestore()
            .collection("user")
    ...
    if (type == "voice") {
        admin
            ...
            .doc(astrologerId)
            .update({
            voiceSeconds: admin.firestore.FieldValue.increment(totalDuration * 60),
        });
        admin
            .firestore()
            .collection("user")
    ...
    if (type == "video") {
        admin
            ...
            .doc(astrologerId)
            .update({
            videoSeconds: admin.firestore.FieldValue.increment(totalDuration * 60),
        });
        admin
            .firestore()
            .collection("user")
...
app.post("/intiatetransaction/", async (req, response) => {
    if (req.body && req.body.user && req.body.orderId && req.body.amount) {
        ...
        PaytmChecksum.generateSignature(JSON.stringify(paytmParams.body), PAYTM_MKEY)
        ...
        if (req.body["STATUS"] == "TXN_SUCCESS") {
            ...
            if (req.body["CASHBACK_ID"] != "") {
                db.collection("cashback")
                    ...
                    .get()
                    .then((cashbackRef) => {
                    addCashbackMoney(paymentData.user, cashbackRef.data().cashback, req.body["ORDERID"], req.body["CASHBACK_ID"]);
                });
            }
...
app.post("/deductAmount/", async (req, res) => {
    ...
    db.collection("meetings")
        ...
        db.collection("astrologer")
            .doc(meetingData.astrologerUid)
            .get()
            .then(async (astrologerRef) => {
            astrologerData = astrologerRef.data();
            db.collection("user")
                .doc(meetingData.userUid)
                .get()
                .then(async (userRef) => {
                ...
                if (meetingData.coupon != null && meetingData.coupon != "") {
                    db.collection("coupon")
                        ...
                        if (couponData != null) {
                            if (couponData.live &&
                                (couponData.categoryType == meetingData.type ||
                                    couponData.categoryType == "all")) {
                                ...
                                db.collection("coupon")
                                    ...
                                    if (useData != null) {
                                        if (useData.useCount < couponData.limit) {
                                            if (useData.totalDiscount + discount >
                                                couponData.maxTotalDiscount) {
                                                discount =
                                                    couponData.maxTotalDiscount -
                                                        useData.totalDiscount;
                                            ...
                                            db.collection("coupon")
                                                ...
                                                totalDiscount: admin.firestore.FieldValue.increment(+discount),
                                            });
                                        }
                                        else {
                                            discount = 0;
                                        ...
                                        db.collection("coupon")
                                            ...
                                            .collection("uses")
                                            .doc(meetingData.userUid)
                                            .set({ useCount: 1, totalDiscount: +discount });
                                        meetingAmount = meetingAmount - discount;
                                        meetingAmount = meetingAmount.toFixed(2);
                                        await deductAmount(meetingAmount, meetingId, meetingData.userUid, res, time);
                                    }
...
app.post("/deductBroadcastAmount/", async (req, res) => {
    ...
    db.collection("broadcasts")
        ...
        db.collection("astrologer")
            .doc(broadcastData.astrologerUid)
            .get()
            .then((astrologerRef) => {
            astrologerData = astrologerRef.data();
            db.collection("user")
...
app.get("/getAgoraToken/", async (req, res) => {
    ...
    const token = RtcTokenBuilder.buildTokenWithUid(appID, appCertificate, channelId, uid, role, privilegeExpiredTs);
...
app.post("/make-call", async (req, res) => {
    ...
    try {
        ...
        admin
            ...
            totalMaxDuration: totalMaxDuration,
...
app.post("/calling-status", async (req, res) => {
    let data = req.body;
    let { callUniqueId, agentNumber, customerNumber, callStatus, recordingUrl, callDuration, callDateAndTime, knowlarityNumber, } = data;
    if (!callUniqueId ||
        !agentNumber ||
        !customerNumber ||
        !callStatus ||
...
sgMail.setApiKey("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");
// const Razorpay = require("razorpay");
// var key_id = "rzp_test_5ViLvtlH2dJ4ku";
// var key_secret = "LOMZVFSYU2ysVvS5O5T7qHpR";
exports.onMeetingUpdate = functions.firestore
    ...
    if ((originalData.status == "cancelled" || originalData.status == "missed") &&
        ...
        admin
            ...
            .update({ status: originalData.status });
    }
    ...
    if (originalData.status == "ongoing" &&
    ...
    if (originalData.status == "Initiated" &&
        ...
        if (updatedData.subtype == "Scheduled") {
            admin
                .firestore()
                .collection("astrologer")
                .doc(updatedData.astrologerUid)
                .collection("slot")
                .doc(updatedData.day)
                .collection("day_slot")
                .doc(updatedData.slotId)
    ...
    if (originalData.status == "ongoing" && updatedData.status == "refunded") {
        amount =
            ((updatedData.lastDuration - updatedData.lastActualDuration) /
                updatedData.lastDuration) *
                updatedData.lastAmountDeduct;
        ...
        status2 = db
            ...
            .collection("wallet_transaction")
            .add({
            subtypeId: context.params.meetingId,
            amount: +amount,
            type: "credit",
            subtype: "meeting",
            date: admin.firestore.FieldValue.serverTimestamp(),
    ...
    if (originalData.status == "ongoing" && updatedData.status == "completed") {
        if (updatedData.userCount < 2) {
            ...
            admin
                ...
                .collection("user")
                .doc(updatedData.userUid)
                .update({ meetingCount: admin.firestore.FieldValue.increment(1) });
            admin
                .firestore()
                .collection("astrologer")
                .doc(updatedData.astrologerUid)
    ...
    if ((originalData.status == "accepted" || originalData.status == "ongoing") &&
        updatedData.status == "cancelled") {
    ...
    if (originalData.rate != updatedData.rate) {
        admin
            ...
            db.collection("astrologer")
                .doc(updatedData.astrologerUid)
                .collection("astrologer_reviews")
                .add({
                description: updatedData.rateMessage,
                rating: updatedData.rate,
                user: {
                    name: userData.firstName,
                    profilePhoto: userData.profilePhotoLink,
                    uuid: updatedData.userUid,
                },
                date: admin.firestore.FieldValue.serverTimestamp(),
            });
        });
...
exports.makeAstrologer = functions.firestore
    ...
    admin
        ...
        .collection("app_details")
        .doc("astrologerDetails")
        .collection("pricing_categories")
        .doc(original["pricingCategory"])
        .get()
        .then((priceRef) => {
        ...
        admin
            ...
            .update({
            priceChat: priceData.priceChat,
            priceVoice: priceData.priceVoice,
            priceVideo: priceData.priceVideo,
            currentDiscount: priceData.currentDiscount,
...
exports.makeuser = functions.firestore
...
exports.createComment = functions.firestore
    .document("/blog/{blogId}/comment/{commentId}")
    .onCreate(async (snap, context) => {
    // Grab the current value of what was written to Firestore.
    const original = snap.data();
    functions.logger.log(original);
...
exports.updateRating = functions.firestore
    .document("/astrologer/{astrologerId}/astrologer_reviews/{reviewId}")
    .onCreate(async (snap, context) => {
    // Grab the current value of what was written to Firestore.
    const original = snap.data();
    functions.logger.log(original);
...
exports.addUserInSecurityGroup = functions.firestore
...
exports.addAstrologerInSecurityGroup = functions.firestore
...
exports.addAdminInSecurityGroup = functions.firestore
...
exports.updateAstrologer = functions.firestore
    ...
    if (originalData.pricingCategory != updatedData.pricingCategory) {
        admin
            ...
            .doc("astrologerDetails")
            .collection("pricing_categories")
            .doc(updatedData.pricingCategory)
            .get()
            .then((priceRef) => {
            priceData = priceRef.data();
            admin
    ...
    if (originalData.status["state"] != updatedData.status["state"] &&
    ...
    else if (originalData.status["state"] != updatedData.status["state"] &&
...
exports.updateWalletWithdrawal = functions.firestore
    ...
    if (originalData.status != updatedData.status) {
        if (updatedData.status == "approved") {
            try {
                if ((updatedData.transactionId != null) &
                    (updatedData.transactionId != "")) {
                    amount = updatedData.amount - updatedData.approvedAmount;
                    admin
                        .firestore()
                        .collection("wallet_withdrawal")
                        .doc(context.params.withdrawalId)
...
exports.updatePricingCategory = functions.firestore
    .document("/app_details/astrologerDetails/pricing_categories/{pricingCategory}")
    .onUpdate(async (change, context) => {
    const updatedData = change.after.data();
    const _datarwt = [];
    await admin
            ...
            admin
                ...
                if (astrologerData.pricingCategory == context.params.pricingCategory) {
                    _datarwt.push(admin
                        ...
                        .doc(doc.ref.id)
                        .update({
                        priceChat: updatedData.priceChat,
                        priceVoice: updatedData.priceVoice,
                        priceVideo: updatedData.priceVideo,
                        currentDiscount: updatedData.currentDiscount,
                        liveChatPrice: updatedData.liveChatPrice,
                    }));
                }
            });
        });
...
exports.scheduledFunctionCrontab = functions.pubsub
    ...
    // });
    const currentDate = admin.firestore.Timestamp.now();
    const startDate = new admin.firestore.Timestamp(currentDate.seconds - 176400, currentDate.nanoseconds).toDate();
    const endDate = new admin.firestore.Timestamp(currentDate.seconds - 86400, currentDate.nanoseconds).toDate();
...
function initiateCall(customerNumber, astrologerNumber, totalMaxDuration) {
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            resolve(Object.assign({}, response.data.success));
        }
        catch (error) {
...
exports.onKnowlarityUpdate = functions.firestore
    ...
    try {
        ...
        admin.firestore().collection("meetings").doc(meetingRef.id).update({
            ...
            totalMaxDuration: totalDurationInSeconds,
